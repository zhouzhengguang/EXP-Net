...
def fine_grained_quant(x, eta, name, INITIAL, value, binary=True):
    ...
    if 'conv' in name:
        ...
        if INITIAL:
            w_s = tf.get_variable('Ws', [(shape[0].value*shape[1].value),1], collections=[tf.GraphKeys.GLOBAL_VARIABLES, 'SCALING'], initializer=tf.constant_initializer(value[name + ':0']))
        else:
            w_s = tf.get_variable('Ws', [(shape[0].value*shape[1].value),1], collections=[tf.GraphKeys.GLOBAL_VARIABLES, 'SCALING'], initializer=tf.constant_initializer(1.0))
        #scalar summary
        ...
        if INITIAL:
            wn = tf.get_variable('Wn', collections=[tf.GraphKeys.GLOBAL_VARIABLES, 'scale_fc'], initializer=value[name + ':0'])
        else:
...
def rows_quant(x, eta, name, INITIAL, value, binary=True):
    ...
    if 'conv' in name:
        if INITIAL:
            w_s = tf.get_variable('Ws', [shape[0].value,1], collections=[tf.GraphKeys.GLOBAL_VARIABLES, 'SCALING'], initializer=tf.constant_initializer(value[name + ':0']))
        else:
